...
public:
	...
	int Signal();
	int Wait (MilliSeconds iWait = INFINITE);

	static int WaitForSingleSignal (Event* pEventArray, unsigned int iNumEvents, MilliSeconds msWait = INFINITE);
	static int WaitForMultipleSignals (Event* pEventArray, unsigned int iNumEvents, MilliSeconds msWait = INFINITE);
};
