...
int Database::Backup (IDatabaseBackupNotificationSink* pSink) {
    ...
    // Sink event
    if (pSink != NULL) {
        pSink->BeginTemplateBackup ((int) m_htTemplates.GetNumElements());
    }
    ...
    // Sink event
    if (pSink != NULL) {
        pSink->BeginTableBackup ((int) m_htTables.GetNumElements());
    }

    // Get number of tables
    iNumTables = (int) m_htTables.GetNumElements();
