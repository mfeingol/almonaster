...
int GameEngine::UpdateTopListOnIncrease(TopListQuery* pQuery)
{
    ...
    iErrCode = t_pCache->GetFirstKey(pszTableName, TopList::EmpireKey, iEmpireKey, &iKey);
    if (iErrCode == OK) {
...
int GameEngine::VerifyTopList (ScoringSystem ssTopList) {
    ...
    for (i = 0; i < iNumRows; i ++) {
        ...
        iErrCode = t_pCache->GetFirstKey(pszTable, TopList::EmpireKey, iEmpireKey, &iTestKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
