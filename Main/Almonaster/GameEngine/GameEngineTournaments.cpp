...
int GameEngine::GetJoinedTournaments(int iEmpireKey, Variant** ppvTournamentKey, Variant** ppvName, unsigned int* piNumTournaments)
{
    ...
    *piNumTournaments = 0;
    ...
    // Read keys
    iErrCode = t_pCache->GetTable(pszEmpTournaments, &pReadTable);
    if (iErrCode != OK) {
...
int GameEngine::DeleteTournament (int iEmpireKey, unsigned int iTournamentKey, bool bOwnerDeleted) {
    ...
    Assert (t_pCache->GetNumRows (pszTable, &iGameClasses) == OK && iGameClasses == 0);
...
int GameEngine::GetTournamentName (unsigned int iTournamentKey, Variant* pvTournamentName) {
    ...
    return t_pCache->ReadData(SYSTEM_TOURNAMENTS, iTournamentKey, SystemTournaments::Name, pvTournamentName);
}
...
int GameEngine::AddEmpireToTournament (unsigned int iTournamentKey, int iInviteKey) {
    ...
    bExists = t_pCache->DoesTableExist (pszTourneys);
    if (!bExists) {
        
        // Fault in table
        iErrCode = t_pCache->CreateTable (pszTourneys, SystemEmpireTournaments::Template);
        if (iErrCode != OK && iErrCode != ERROR_TABLE_ALREADY_EXISTS) {
            t_pCache->DeleteRow(pszEmpires, iProxyKey);
            Assert (false);
            goto Cleanup;
        }
    }
    ...
    iErrCode = t_pCache->InsertRow (pszTourneys, SystemEmpireTournaments::Template, &vVal, NULL);
    if (iErrCode != OK) {
        t_pCache->DeleteRow(pszEmpires, iProxyKey);
        Assert (false);
        goto Cleanup;
...
int GameEngine::GetTournamentTeams (unsigned int iTournamentKey, unsigned int** ppiTeamKey, Variant** ppvName, 
    ...
    if (ppvName == NULL) {
        ...
        if (ppiTeamKey == NULL) {
            iErrCode = t_pCache->GetNumRows (pszTeams, piNumKeys);
        } else {
...
int GameEngine::SetEmpireTournamentTeam (unsigned int iTournamentKey, int iEmpireKey, unsigned int iTeamKey) {
    ...
    if (iTeamKey != NO_KEY) {
        ...
        iErrCode = t_pCache->DoesRowExist(pszTeams, iTeamKey, &bExists);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetTournamentGames (unsigned int iTournamentKey, int** ppiGameClass, int** ppiGameNumber, 
    ...
    if (ppiGameClass == NULL) {
        return t_pCache->GetNumRows (pszGames, piNumGames);
    }
