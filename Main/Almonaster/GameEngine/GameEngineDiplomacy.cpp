...
int GameEngine::GetDiplomaticOptions (int iGameClass, int iGameNumber, int iEmpireKey, int iFoeKey, 
    ...
    *piNumOptions = 0;
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strDip, iKey, GameEmpireDiplomacy::DipOffer, &vWeOffer);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    *piSelected = vWeOffer.GetInteger();
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strDip, iKey, GameEmpireDiplomacy::CurrentStatus, &vStatus);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (vStatus.GetInteger() == WAR && 
        ...
        if (iNumEmps > 2) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strDip, iKey, GameEmpireDiplomacy::State, &vState);
            if (iErrCode != OK) {
                Assert (false);
                return iErrCode;
            }
...
int GameEngine::AddDiplomaticOption (int iGameClass, int iGameNumber, int iTargetEmpireKey,
    ...
    if (iDiplomacyLevel == iCurrentDiplomacyLevel) {
        ...
        if (iMaxCount == UNRESTRICTED_DIPLOMACY) {
            ...
            Variant vValue;
            int iErrCode = t_pConn->GetCache()->ReadData(pszEmpireData, pszEmpireDataColumn, &vValue);
            if (iErrCode != OK) {
                Assert (false);
                return iErrCode;
            }
...
int GameEngine::UpdateDiplomaticOffer (int iGameClass, int iGameNumber, int iEmpireKey, int iFoeKey, 
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireDiplomacy, iKey, GameEmpireDiplomacy::CurrentStatus, &vStatus);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }

    iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireDiplomacy, iKey, GameEmpireDiplomacy::DipOffer, &vOffering);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (iDipOffer == WAR) {
        ...
        if (iDipOffer > vStatus.GetInteger()) {
            ...
            if (iMax != UNRESTRICTED_DIPLOMACY) {
                ...
                iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireData, pszColumn, &vCount);
                if (iErrCode != OK) {
                    return iErrCode;
                }
...
int GameEngine::GetLastUsedMessageTarget (int iGameClass, int iGameNumber, int iEmpireKey, int* piLastUsedMask,
    ...
    *piNumLastUsed = 0;
    ...
    // Read mask
    iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireData, GameEmpireData::LastMessageTargetMask, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
