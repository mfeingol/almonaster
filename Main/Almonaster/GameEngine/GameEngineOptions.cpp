...
int GameEngine::SetEmpireOption (int iGameClass, int iGameNumber, int iEmpireKey, unsigned int iFlag, 
    ...
    if (bOption) {
        return t_pConn->WriteOr (strEmpireOptions, GameEmpireData::Options, iFlag);
    } else {
        return t_pConn->WriteAnd (strEmpireOptions, GameEmpireData::Options, ~iFlag);
    }
}
...
int GameEngine::SetEmpireMaxNumSavedGameMessages (int iGameClass, int iGameNumber, int iEmpireKey, 
    ...
    iErrCode = t_pConn->WriteData (strOptions, GameEmpireData::MaxNumGameMessages, iMaxNumSavedMessages);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::SetEmpireDefaultMessageTarget (int iGameClass, int iGameNumber, int iEmpireKey, 
    ...
    return t_pConn->WriteData (strGameEmpireData, GameEmpireData::DefaultMessageTarget, iMessageTarget);
}
...
int GameEngine::RequestPauseDuringUpdate (int iGameClass, int iGameNumber, int iEmpireKey) {
    ...
    iErrCode = t_pConn->WriteOr (strGameEmpireData, GameEmpireData::Options, REQUEST_PAUSE);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::RequestPause (int iGameClass, int iGameNumber, int iEmpireKey, int* piGameState) {
    ...
    iErrCode = t_pConn->Increment (strGameData, GameData::NumRequestingPause, 1, &vOldNum);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::RequestNoPause (int iGameClass, int iGameNumber, int iEmpireKey, int* piGameState) {
    ...
    iErrCode = t_pConn->WriteAnd (strGameEmpireData, GameEmpireData::Options, ~REQUEST_PAUSE);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::RequestDraw (int iGameClass, int iGameNumber, int iEmpireKey, int* piGameState) {
    ...
    iErrCode = t_pConn->WriteOr (strGameEmpireData, GameEmpireData::Options, REQUEST_DRAW);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }

    iErrCode = t_pConn->Increment (strGameData, GameData::NumRequestingDraw, 1, &vOldNum);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::RequestNoDraw (int iGameClass, int iGameNumber, int iEmpireKey) {
    ...
    iErrCode = t_pConn->WriteAnd (strGameEmpireData, GameEmpireData::Options, ~REQUEST_DRAW);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
