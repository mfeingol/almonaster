...
int GameEngine::CheckGameForUpdates (int iGameClass, int iGameNumber, bool fUpdateCheckTime, bool* pbUpdate) {
    ...
    *pbUpdate = false;
    ...
    if (!(iGameState & PAUSED)) {
        ...
        iErrCode = t_pConn->ReadData (strGameData, GameData::NumUpdates, &vNumPrevUpdates);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (!bGameOver && !(iGameState & STILL_OPEN)) {
        ...
        while (!bGameOver) {
            ...
            iErrCode = t_pConn->ReadData (strGameData, GameData::NumEmpiresUpdated, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
...
int GameEngine::GetGameUpdateData (int iGameClass, int iGameNumber, int* piSecondsSince, int* piSecondsUntil, 
    ...
    if (!(iGameState & STARTED)) {
        ...
        iErrCode = t_pConn->ReadData (strGameData, GameData::FirstUpdateDelay, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
    ...
    iErrCode = t_pConn->ReadData (strGameData, GameData::RealLastUpdateTime, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if ((iGameState & ADMIN_PAUSED) || (iGameState & PAUSED)) {
        ...
        iErrCode = t_pConn->ReadData (strGameData, GameData::SecondsUntilNextUpdateWhilePaused, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
...
int GameEngine::VerifyUpdatedEmpireCount (int iGameClass, int iGameNumber) {
    ...
    while (true) {
        ...
        iErrCode = t_pConn->ReadData (pszGameEmpires, iKey, GameEmpires::EmpireKey, &vEmpireKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
