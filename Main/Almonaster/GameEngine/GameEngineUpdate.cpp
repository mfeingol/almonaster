...
int GameEngine::CheckGameForUpdates (int iGameClass, int iGameNumber, bool fUpdateCheckTime, bool* pbUpdate) {
    ...
    *pbUpdate = false;
    ...
    if (!(iGameState & PAUSED)) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::NumUpdates, &vNumPrevUpdates);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (!bGameOver && !(iGameState & STILL_OPEN)) {
        ...
        while (!bGameOver) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::NumEmpiresUpdated, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->GetNumRows (pszGameEmpires, &iNumNeeded);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
...
Cleanup:
    ...
    if (fUpdateCheckTime && !bGameOver && iErrCode == OK) {
        ...
        // Update last checked
        iErrCode = t_pConn->WriteData (strGameData, GameData::LastUpdateCheck, tNow);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetGameUpdateData (int iGameClass, int iGameNumber, int* piSecondsSince, int* piSecondsUntil, 
    ...
    if (!(iGameState & STARTED)) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::FirstUpdateDelay, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::RealLastUpdateTime, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if ((iGameState & ADMIN_PAUSED) || (iGameState & PAUSED)) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::SecondsUntilNextUpdateWhilePaused, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
...
int GameEngine::SetEmpireReadyForUpdate (int iGameClass, int iGameNumber, int iEmpireKey, bool* pbSet) {
    ...
    *pbSet = (vOptions.GetInteger() & UPDATED) == 0;
    if (*pbSet) {
        ...
        iErrCode = t_pConn->WriteOr (strGameEmpireData, GameEmpireData::Options, UPDATED);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strGameData, GameData::NumEmpiresUpdated, 1);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::SetEmpireNotReadyForUpdate (int iGameClass, int iGameNumber, int iEmpireKey, bool* pbSet) {
    ...
    *pbSet = (vOptions.GetInteger() & UPDATED) != 0;
    if (*pbSet) {
        ...
        iErrCode = t_pConn->WriteAnd (strEmpireData, GameEmpireData::Options, ~UPDATED);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::VerifyUpdatedEmpireCount (int iGameClass, int iGameNumber) {
    ...
    while (true) {
        ...
        iErrCode = t_pConn->GetNextKey (pszGameEmpires, iKey, &iKey);
        if (iErrCode == ERROR_DATA_NOT_FOUND) {
            iErrCode = OK;
            break;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(pszGameEmpires, iKey, GameEmpires::EmpireKey, &vEmpireKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
