                                 ...
                                 const char* pszProperty, Variant* pvFleetName)
{
    ...
    iErrCode = t_pConn->GetTableForReading(strGameEmpireFleets, &pFleets);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetFleetFlag (int iGameClass, int iGameNumber, int iEmpireKey, int iFleetKey, int iFlag, 
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireFleets, iFleetKey, GameEmpireFleets::Flags, &vTemp);
    if (iErrCode != OK) {
        return iErrCode;
    }
...
int GameEngine::SetFleetFlag (int iGameClass, int iGameNumber, int iEmpireKey, int iFleetKey, int iFlag, 
    ...
    iErrCode = t_pConn->GetTableForWriting (strGameEmpireFleets, &pFleets);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetNewFleetLocations (int iGameClass, int iGameNumber, int iEmpireKey, int** ppiLocationKey, 
    ...
    *piNumLocations = 0;
    ...
    iErrCode = t_pConn->GetTableForReading(strEmpireMap, &pEmpireMap);
    if (iErrCode != OK) {
        return iErrCode;
    }
    ...
    for (i = 0; i < iNumPlanets; i ++) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireMap, piProxyKey[i], GameEmpireMap::NumUncloakedShips, &vTemp);
        if (iErrCode != OK) {
            goto Cleanup;
        }
        ...
        if (vTemp.GetInteger() > 0) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strEmpireMap, piProxyKey[i], GameEmpireMap::NumCloakedShips, &vTemp);
            if (iErrCode != OK) {
                goto Cleanup;
            }
            ...
            if (vTemp.GetInteger() > 0) {
                ...
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piPlanetKey[i], GameMap::Owner, &vTemp);
                if (iErrCode != OK) {
                    goto Cleanup;
                }
                ...
                if (vTemp.GetInteger() == iEmpireKey) {
                    ...
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piPlanetKey[i], GameMap::Pop, &vTemp);
...
int GameEngine::CreateNewFleet (int iGameClass, int iGameNumber, int iEmpireKey, const char* pszFleetName, 
    ...
    *piFleetKey = NO_KEY;
    ...
    unsigned int iKey;
    iErrCode = t_pConn->GetFirstKey (strEmpireFleets, GameEmpireFleets::Name, pszFleetName, &iKey);
    ...
    // Insert new row
    {
        ...
        if (pvColVal[GameEmpireFleets::iName].GetCharPtr() == NULL) {
            iErrCode = ERROR_OUT_OF_MEMORY;
            goto Cleanup;
        }
        
        // Insert row into fleets table
        iErrCode = t_pConn->InsertRow(strEmpireFleets, GameEmpireFleets::Template, pvColVal, piFleetKey);
        if (iErrCode != OK) {
            goto Cleanup;
        }
    }
...
int GameEngine::GetFleetOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piSelected = 0;
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireFleets, iFleetKey, GameEmpireFleets::Action, &vTemp);
    if (iErrCode != OK) {
        goto Cleanup;
    }
    ...
    // Get fleet location
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vTemp);
    if (iErrCode != OK) {
        goto Cleanup;
    }
    ...
    if (iNumShips > 0) {
        ...
        iErrCode = t_pConn->GetTableForReading(strEmpireFleets, &pFleets);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetFleetSpecialActionMask (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piMask = 0;
    ...
    sc[1].pszColumn = GameEmpireShips::BuiltThisUpdate;
    sc[1].iFlags = 0;
    sc[1].vData = 0;
    sc[1].vData2 = 0;
    ...
    iErrCode = t_pConn->GetSearchKeys (
        pszShips,
        sd,
        &piShipKey,
        &iNumShips,
    ...
    // Get planet key
    iErrCode = t_pConn->GetCache()->ReadData(pszFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    for (i = 0; i < iNumShips; i ++) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(pszShips, piShipKey[i], GameEmpireShips::Type, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(pszShips, piShipKey[i], GameEmpireShips::State, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::UpdateFleetName (int iGameClass, int iGameNumber, int iEmpireKey, int iFleetKey, 
    ...
    bool bFleetExists;
    if (t_pConn->DoesRowExist (strEmpireFleets, iFleetKey, &bFleetExists) != OK || !bFleetExists) {
        return ERROR_FLEET_DOES_NOT_EXIST;
    }

    // Write new name
    return t_pConn->WriteData (strEmpireFleets, iFleetKey, GameEmpireFleets::Name, pszNewName);
}
...
int GameEngine::UpdateFleetOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    iErrCode = t_pConn->DoesRowExist (strEmpireFleets, iFleetKey, &bFleetExists);
    if (iErrCode != OK || !bFleetExists) {
        iErrCode = ERROR_FLEET_DOES_NOT_EXIST;
        goto Cleanup;
    }

    iErrCode = t_pConn->GetCache()->ReadData(strEmpireFleets, iFleetKey, GameEmpireFleets::Action, &vOldAction);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iOrderKey == STAND_BY) {
        ...
        iErrCode = t_pConn->WriteData (strEmpireFleets, iFleetKey, GameEmpireFleets::Action, STAND_BY);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (iOrderKey == DISBAND || iOrderKey == DISBAND_AND_DISMANTLE) {
        ...
        iErrCode = t_pConn->DeleteRow (strEmpireFleets, iFleetKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vFleetPlanet);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iOrderKey == NUKE) {
        ...
        // Get owner
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, vFleetPlanet, GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (iOwner != INDEPENDENT) {
            ...
            unsigned int iKey;
            iErrCode = t_pConn->GetFirstKey(strDiplomacy, GameEmpireDiplomacy::EmpireKey, iOwner, &iKey);
            if (iErrCode != OK) {
                iErrCode = ERROR_CANNOT_NUKE;
                goto Cleanup;
            }
            
            iErrCode = t_pConn->GetCache()->ReadData(strDiplomacy, iKey, GameEmpireDiplomacy::CurrentStatus, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
        ...
        iErrCode = t_pConn->WriteData (strEmpireFleets, iFleetKey, GameEmpireFleets::Action, NUKE);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (IS_FLEET_STANDBY_ACTION (iOrderKey)) {
        ...
        iErrCode = t_pConn->WriteData (strEmpireFleets, iFleetKey, GameEmpireFleets::Action, iOrderKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    ENUMERATE_CARDINAL_POINTS(i) {
        ...
        if (iOrderKey == MOVE_NORTH - (int) i) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strEmpireFleets, iFleetKey, GameEmpireFleets::BuildShips, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, vFleetPlanet, GameMap::Link, &vLink);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;;
            }
...
int GameEngine::GetNumShipsInFleet (int iGameClass, int iGameNumber, int iEmpireKey, int iFleetKey, 
    ...
    iErrCode = t_pConn->GetTableForReading(pszFleets, &pFleets);
    if (iErrCode != OK) {
        return iErrCode;
    }
...
int GameEngine::MergeFleets (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey,
    ...
    iErrCode = t_pConn->DeleteRow (pszFleets, iSrcKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (vSrcName.GetType() == V_STRING) {
        ...
        iErrCode = t_pConn->GetTableForWriting (pszFleets, &pWriteNewFleet);
        if (iErrCode == OK) {
