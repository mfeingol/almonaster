...
int GameEngine::GetFleetFlag (int iGameClass, int iGameNumber, int iEmpireKey, int iFleetKey, int iFlag, 
    ...
    iErrCode = t_pConn->ReadData (strGameEmpireFleets, iFleetKey, GameEmpireFleets::Flags, &vTemp);
    if (iErrCode != OK) {
        return iErrCode;
    }
...
int GameEngine::GetNewFleetLocations (int iGameClass, int iGameNumber, int iEmpireKey, int** ppiLocationKey, 
    ...
    *piNumLocations = 0;
    ...
    for (i = 0; i < iNumPlanets; i ++) {
        ...
        iErrCode = t_pConn->ReadData (strEmpireMap, piProxyKey[i], GameEmpireMap::NumUncloakedShips, &vTemp);
        if (iErrCode != OK) {
            goto Cleanup;
        }
        ...
        if (vTemp.GetInteger() > 0) {
            ...
            iErrCode = t_pConn->ReadData (strEmpireMap, piProxyKey[i], GameEmpireMap::NumCloakedShips, &vTemp);
            if (iErrCode != OK) {
                goto Cleanup;
            }
            ...
            if (vTemp.GetInteger() > 0) {
                ...
                iErrCode = t_pConn->ReadData (strGameMap, piPlanetKey[i], GameMap::Owner, &vTemp);
                if (iErrCode != OK) {
                    goto Cleanup;
                }
                ...
                if (vTemp.GetInteger() == iEmpireKey) {
                    ...
                    iErrCode = t_pConn->ReadData (strGameMap, piPlanetKey[i], GameMap::Pop, &vTemp);
...
int GameEngine::GetFleetOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piSelected = 0;
    ...
    iErrCode = t_pConn->ReadData (strEmpireFleets, iFleetKey, GameEmpireFleets::Action, &vTemp);
    if (iErrCode != OK) {
        goto Cleanup;
    }
    ...
    // Get fleet location
    iErrCode = t_pConn->ReadData (strEmpireFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vTemp);
    if (iErrCode != OK) {
        goto Cleanup;
    }
...
int GameEngine::GetFleetSpecialActionMask (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piMask = 0;
    ...
    // Get planet key
    iErrCode = t_pConn->ReadData (pszFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    for (i = 0; i < iNumShips; i ++) {
        ...
        iErrCode = t_pConn->ReadData (pszShips, piShipKey[i], GameEmpireShips::Type, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->ReadData (pszShips, piShipKey[i], GameEmpireShips::State, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::UpdateFleetOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    iErrCode = t_pConn->ReadData (strEmpireFleets, iFleetKey, GameEmpireFleets::Action, &vOldAction);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->ReadData (strEmpireFleets, iFleetKey, GameEmpireFleets::CurrentPlanet, &vFleetPlanet);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iOrderKey == NUKE) {
        ...
        // Get owner
        iErrCode = t_pConn->ReadData (strGameMap, vFleetPlanet, GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (iOwner != INDEPENDENT) {
            ...
            iErrCode = t_pConn->ReadData(strDiplomacy, iKey, GameEmpireDiplomacy::CurrentStatus, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
    ...
    ENUMERATE_CARDINAL_POINTS(i) {
        ...
        if (iOrderKey == MOVE_NORTH - (int) i) {
            ...
            iErrCode = t_pConn->ReadData (strEmpireFleets, iFleetKey, GameEmpireFleets::BuildShips, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->ReadData (strGameMap, vFleetPlanet, GameMap::Link, &vLink);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;;
            }
