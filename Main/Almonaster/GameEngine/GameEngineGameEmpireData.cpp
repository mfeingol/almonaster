...
int GameEngine::DeleteEmpireFromGame (int iGameClass, int iGameNumber, int iEmpireKey, ReasonForRemoval rReason,
    ...
    // Get game state
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::State, &vGameState);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::Options, &vOptions);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (vOptions.GetInteger() & UPDATED) {
        ...
        iErrCode = t_pConn->Increment (strGameData, GameData::NumEmpiresUpdated, -1, &vTemp);
        Assert (iErrCode == OK);
    }
    ...
    if (vOptions.GetInteger() & REQUEST_PAUSE) {
        ...
        iErrCode = t_pConn->Increment (strGameData, GameData::NumRequestingPause, -1);
        Assert (iErrCode == OK);
    }

    // Handle num drawing
    if (vOptions.GetInteger() & REQUEST_DRAW) {
        iErrCode = t_pConn->Increment (strGameData, GameData::NumRequestingDraw, -1);
        Assert (iErrCode == OK);
    }
    
    // Handle num resigned
    if (vOptions.GetInteger() & RESIGNED) {
        iErrCode = t_pConn->Increment (strGameData, GameData::NumEmpiresResigned, -1);
        Assert (iErrCode == OK);
    }
    ...
    iErrCode = t_pConn->GetFirstKey (strEmpires, GameEmpires::EmpireKey, iEmpireKey, &iKey);
    if (iKey == NO_KEY) {
        Assert (false);
        return ERROR_FAILURE;
    }

    iErrCode = t_pConn->DeleteRow (strEmpires, iKey);
    if (iKey == NO_KEY) {
        Assert (false);
        return ERROR_FAILURE;
    }
        ...
        if (GetEmpireName (iEmpireKey, pvTemp + GameDeadEmpires::iName) == OK) {
            ...
            if (GetEmpireProperty (iEmpireKey, SystemEmpireData::AlienKey, &vIcon) == OK) {
                ...
                if (GetEmpireProperty(iEmpireKey, SystemEmpireData::SecretKey, pvTemp + GameDeadEmpires::iSecretKey) == OK) {
                    ...
                    iErrCode = t_pConn->InsertRow (strGameEmpireData, GameDeadEmpires::Template, pvTemp, NULL);
                    Assert (iErrCode == OK);
                }
            }
        }
    ...
    Variant* pvEmpireKey;
    iErrCode = t_pConn->ReadColumn (strEmpires, GameEmpires::EmpireKey, &pvEmpireKey, &iNumEmpires);
    ...
    if (iErrCode == OK) {
        ...
        for (i = 0; i < iNumEmpires; i ++) {    
            ...
            if (iErrCode == OK) {
                ...
                iErrCode = t_pConn->GetCache()->ReadData(strOtherEmpireDip, iKey, GameEmpireDiplomacy::DipOffer, &vDipOffer);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->GetCache()->ReadData(strOtherEmpireDip, iKey, GameEmpireDiplomacy::CurrentStatus, &vDipStatus);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->GetCache()->ReadData(strOtherEmpireDip, iKey, GameEmpireDiplomacy::State, &vState);
                Assert (iErrCode == OK);

                // Nuke the row
                iErrCode = t_pConn->DeleteRow (strOtherEmpireDip, iKey);
                Assert (iErrCode == OK);
                ...
                case ALLIANCE:
                    ...
                    if (!bPermanentAlliances) {
                        ...
                        iErrCode = t_pConn->Increment (strGameEmpireData, GameEmpireData::NumAlliancesLeaked, 1);
                        Assert (iErrCode == OK);
                    }
    ...
    int iMaxPop;
    iErrCode = t_pConn->GetEqualKeys (strGameMap, GameMap::Owner, iEmpireKey, &piPlanetKey, &iNumKeys);
    ...
    if (iErrCode == OK) {
        ...
        for (i = 0; i < iNumKeys; i ++) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piPlanetKey[i], GameMap::Ag, &vAg);
            Assert (iErrCode == OK);

            if (bIndependence && vAg.GetInteger() > 0) {

                // Make planet independent
                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::Owner, INDEPENDENT);
                Assert (iErrCode == OK);

                // Set max pop to ag
                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::MaxPop, vAg);
                Assert (iErrCode == OK);
                ...
                // Set max pop
                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::Pop, vPop);
                Assert (iErrCode == OK);

            } else {

                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::Owner, SYSTEM);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::Pop, 0);
                Assert (iErrCode == OK);

                // Set max pop to the right thing
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piPlanetKey[i], GameMap::Minerals, &vMin);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piPlanetKey[i], GameMap::Fuel, &vFuel);
                Assert (iErrCode == OK);
                ...
                iErrCode = t_pConn->WriteData (strGameMap, piPlanetKey[i], GameMap::MaxPop, iMaxPop);
                Assert (iErrCode == OK);
            }
        }
    ...
    if (t_pConn->GetNumRows (strGameMap, &iNumRows) == OK && iNumRows > 0) {

        Variant vHWKey, vStillHW;
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::HomeWorld, &vHWKey);
        if (iErrCode == OK && vHWKey.GetInteger() != NO_KEY) {

            unsigned int iHWKey = vHWKey.GetInteger();

            // Is the homeworld still a homeworld?
            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iHWKey, GameMap::HomeWorld, &vStillHW);
            Assert (iErrCode == OK);
            ...
            if (vStillHW.GetInteger() == HOMEWORLD) {
                ...
                iErrCode = t_pConn->WriteData (strGameMap, iHWKey, GameMap::HomeWorld, NOT_HOMEWORLD);
                Assert (iErrCode == OK);

                // Halve resources
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iHWKey, GameMap::Ag, &vAg);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iHWKey, GameMap::Minerals, &vMin);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iHWKey, GameMap::Fuel, &vFuel);
                Assert (iErrCode == OK);

                iErrCode = t_pConn->WriteData (strGameMap, iHWKey, GameMap::Ag, (int) (vAg.GetInteger() / 2));
                Assert (iErrCode == OK);

                iErrCode = t_pConn->WriteData (strGameMap, iHWKey, GameMap::MaxPop, (int) (vAg.GetInteger() / 2));
                Assert (iErrCode == OK);

                iErrCode = t_pConn->WriteData (strGameMap, iHWKey, GameMap::Fuel, (int) (vFuel.GetInteger() / 2));
                Assert (iErrCode == OK);
                
                iErrCode = t_pConn->WriteData (strGameMap, iHWKey, GameMap::Minerals, (int) (vMin.GetInteger() / 2));
                Assert (iErrCode == OK);
            }
        }
    }
    ...
    iErrCode = t_pConn->ReadColumn (strEmpireShips, GameEmpireShips::CurrentPlanet, &piShipKey, &pvPlanetKey, &iNumShips);
    Assert (iErrCode == OK || iErrCode == ERROR_DATA_NOT_FOUND);
    ...
    if (iNumShips > 0) {
        ...
        if (!bIndependence) {
            ...
            for (i = 0; i < iNumShips; i ++) {
                ...
                // Get owner of planet
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, pvPlanetKey[i], GameMap::Owner, &vOwner);
                Assert (iErrCode == OK);
                ...
                case SYSTEM:
                    ...
                    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, piShipKey[i], GameEmpireShips::Type, &vType);
                    if (iErrCode == OK) {
                        ...
                        if (vType.GetInteger() == COLONY) {
                            ...
                            // Read planet's ag
                            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, pvPlanetKey[i], GameMap::Ag, &vAg);
                            ...
                            if (vAg.GetInteger() > 0) {
                                ...
                                if (vOwner.GetInteger() != INDEPENDENT) {
                                    ...
                                    iErrCode = t_pConn->WriteData (strGameMap, pvPlanetKey[i], GameMap::Owner, INDEPENDENT);
                                    Assert (iErrCode == OK);

                                    iErrCode = t_pConn->WriteData (strGameMap, pvPlanetKey[i], GameMap::MaxPop, vAg);
                                    Assert (iErrCode == OK);
                                }
                                
                                // Get colony's capacity
                                iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, piShipKey[i], GameEmpireShips::CurrentBR, &vBR);
                                if (iErrCode == OK) {
                                    ...
                                    if ((int) iInitPop <= vAg.GetInteger()) {
                                        ...
                                        // Reduce ship BR
                                        iErrCode = t_pConn->Increment (strEmpireShips, piShipKey[i], GameEmpireShips::CurrentBR, - fDecrease);
                                        Assert (iErrCode == OK);
                                    ...
                                    // Deposit population
                                    iErrCode = t_pConn->Increment (strGameMap, pvPlanetKey[i], GameMap::Pop, iInitPop);
                                    Assert (iErrCode == OK);
                ...
                if (iOwnerKey == NO_KEY) {
                    ...
                    // Read ship data
                    iErrCode = t_pConn->ReadRow (strEmpireShips, piShipKey[i], &pvData);
                    if (iErrCode == OK) {
                        ...
                        if (pvData[GameEmpireShips::iState].GetInteger() & CLOAKED) {
                            ...
                            if (bBuilding) {
                                ...
                                iErrCode = t_pConn->Increment (strGameMap, pvPlanetKey[i], GameMap::NumCloakedBuildShips, -1);
                                Assert (iErrCode == OK);

                            } else {

                                iErrCode = t_pConn->Increment (strGameMap, pvPlanetKey[i], GameMap::NumCloakedShips, -1);
                                Assert (iErrCode == OK);
                            }

                            iErrCode = t_pConn->Increment (strGameMap, pvPlanetKey[i], GameMap::NumUncloakedShips, 1);
                            Assert (iErrCode == OK);
                            ...
                            if (bBuilding) {
                                ...
                                iErrCode = t_pConn->Increment (strGameMap, pvPlanetKey[i], GameMap::NumUncloakedBuildShips, -1);
                                Assert (iErrCode == OK);
                        ...
                        if (iOwnerKey == INDEPENDENT) {
                            ...
                            iErrCode = t_pConn->InsertRow (strIndependentShips, GameIndependentShips::Template, pvData, NULL);
                            Assert (iErrCode == OK);
                            ...
                            iErrCode = t_pConn->InsertRow (strGameEmpireShips, GameEmpireShips::Template, pvData, NULL);
                            Assert (iErrCode == OK);
    ...
    Variant vMax, vOld;
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::MaxEcon, &vMax);
    Assert (iErrCode == OK);

    iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::MaxEcon, &vOld);
    Assert (iErrCode == OK);
    
    if (vMax.GetInteger() > vOld.GetInteger())
    {
        iErrCode = t_pConn->WriteData (strSystemEmpireData, iEmpireKey, SystemEmpireData::MaxEcon, vMax);
        Assert (iErrCode == OK);
    }
    
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::MaxMil, &vMax);
    Assert (iErrCode == OK);

    iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::MaxMil, &vOld);
    Assert (iErrCode == OK);
    
    if (vMax.GetInteger() > vOld.GetInteger())
    {
        iErrCode = t_pConn->WriteData (strSystemEmpireData, iEmpireKey, SystemEmpireData::MaxMil, vMax);
        Assert (iErrCode == OK);
    }
    ...
    // GameEmpireData(I.I.I)
    iErrCode = t_pConn->DeleteTable (strEmpireData);
    Assert (iErrCode == OK);
    ...
    iErrCode = t_pConn->DeleteTable (strEmpireShips);
    Assert (iErrCode == OK);
    ...
    if (iGames == 0) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::Options, &vOld);
        Assert (iErrCode == OK);

        if (iErrCode == OK && (vOld.GetInteger() & EMPIRE_MARKED_FOR_DELETION)) {

            iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::SecretKey, &vOld);
            Assert (iErrCode == OK);
...
int GameEngine::RemoveEmpireFromGame (int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    iErrCode = t_pConn->WriteOr (strGameData, GameData::State, GAME_DELETING_EMPIRE);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->WriteAnd (strGameData, GameData::State, ~GAME_DELETING_EMPIRE);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (bGameAlive) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::NumUpdates, &vState);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
...
int GameEngine::QuitEmpireFromGameInternal (int iGameClass, int iGameNumber, int iEmpireKey, int iKillerEmpire) {
    ...
    Variant vStarted;
    int iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::State, &vStarted);
    if (iErrCode != OK) {
        return ERROR_GAME_DOES_NOT_EXIST;
    }
    ...
    if (iNumEmpires == 1) {
        ...
        int iErrCode2 = t_pConn->WriteAnd (strGameData, GameData::State, ~GAME_DELETING_EMPIRE);
...
int GameEngine::ResignEmpireFromGame (int iGameClass, int iGameNumber, int iEmpireKey) {
    ...
    iErrCode = t_pConn->WriteOr (strEmpireData, GameEmpireData::Options, RESIGNED);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->Increment (strGameData, GameData::NumEmpiresResigned, 1, &vOldResigned);
    if (iErrCode != OK) {
        
        Assert (false);
        
        iErrCode = t_pConn->WriteAnd (strEmpireData, GameEmpireData::Options, ~RESIGNED);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    while (true) {
        ...
        iErrCode = t_pConn->GetNextKey (strEmpireShips, iShipKey, &iShipKey);
        if (iErrCode != OK) {
...
int GameEngine::UnresignEmpire (int iGameClass, int iGameNumber, int iEmpireKey, int iAdminKey) {
    ...
    if (vOptions.GetInteger() & RESIGNED) {
        ...
        iErrCode = t_pConn->Increment (strGameData, GameData::NumEmpiresResigned, -1);
        if (iErrCode != OK) {
            int iErrCode2 = t_pConn->WriteOr (strEmpireData, GameEmpireData::Options, RESIGNED);
            Assert (iErrCode2 == OK);
            goto Cleanup;
        }
...
int GameEngine::SurrenderEmpireFromGame (int iGameClass, int iGameNumber, int iEmpireKey, SurrenderType sType) {
    ...
    // Get update count
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::NumUpdates, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (sType == SC30_SURRENDER) {
        ...
        iErrCode = t_pConn->WriteData (strGameMap, vHomeWorld.GetInteger(), GameMap::HomeWorld, iEmpireKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->WriteData (strGameMap, vHomeWorld.GetInteger(), GameMap::Name, pszString);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (iNumEmpires == 2) {
        ...
        while (true) {
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameEmpires, iKey, GameEmpires::EmpireKey, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
...
int GameEngine::DeleteShipFromDeadEmpire (const char* pszEmpireShips, const char* pszGameMap, 
    ...
    iErrCode = t_pConn->GetCache()->ReadData(pszEmpireShips, iShipKey, GameEmpireShips::State, &vState);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }

    iErrCode = t_pConn->GetCache()->ReadData(pszEmpireShips, iShipKey, GameEmpireShips::BuiltThisUpdate, &vBuilt);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (vBuilt.GetInteger() != 0) {
        ...
        if (vState.GetInteger() & CLOAKED) {
            iErrCode = t_pConn->Increment (pszGameMap, iPlanetKey, GameMap::NumCloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                return iErrCode;
            }
        } else {
            iErrCode = t_pConn->Increment (pszGameMap, iPlanetKey, GameMap::NumUncloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                return iErrCode;
            }
        ...
        if (vState.GetInteger() & CLOAKED) {
            iErrCode = t_pConn->Increment (pszGameMap, iPlanetKey, GameMap::NumCloakedShips, -1);
            Assert (iErrCode == OK);
        } else {
            iErrCode = t_pConn->Increment (pszGameMap, iPlanetKey, GameMap::NumUncloakedShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                return iErrCode;
            }
#ifdef _DEBUG
            Variant vFooBar;
            iErrCode = t_pConn->GetCache()->ReadData(pszGameMap, iPlanetKey, GameMap::NumUncloakedShips, &vFooBar);
...
int GameEngine::GetEmpirePartialMapData (int iGameClass, int iGameNumber, int iEmpireKey, bool* pbPartialMaps, 
    ...
    int iErrCode = t_pConn->GetTableForReading(pszGameEmpireData, &pGameEmpireData);
    if (iErrCode != OK) {
        goto Cleanup;
    }
...
int GameEngine::GetEmpireMaintenanceRatio (int iGameClass, int iGameNumber, int iEmpireKey, 
    ...
    int iErrCode = t_pConn->GetTableForReading(strGameEmpireData, &pGameEmpireData);
    if (iErrCode != OK) {
        return iErrCode;
    }
...
int GameEngine::WriteNextStatistics (int iGameClass, int iGameNumber, int iEmpireKey, int iTotalAg, 
    ...
    Variant vTargetPop;
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TargetPop, &vTargetPop);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->WriteData (strEmpireData, GameEmpireData::NextTotalPop, iNextPop);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->WriteData (strEmpireData, GameEmpireData::NextMin, iNextMin);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->WriteData (strEmpireData, GameEmpireData::NextFuel, iNextFuel);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
void GameEngine::CheckTargetPop (int iGameClass, int iGameNumber, int iEmpireKey) {
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TotalAg, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::BonusAg, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::UpdateGameEmpireString (int iGameClass, int iGameNumber, int iEmpireKey, const char* pszColumn, 
    ...
    *pbTruncated = false;
    ...
    iErrCode = t_pConn->GetTableForWriting (pszGameEmpireData, &pWriteTable);
    if (iErrCode != OK) {
        goto Cleanup;
    }
...
int GameEngine::GetEmpireGameProperty (int iGameClass, int iGameNumber, int iEmpireKey, const char* pszColumn, Variant* pvProperty) {
    ...
    return t_pConn->GetCache()->ReadData(strGameEmpireData, pszColumn, pvProperty);
}
...
int GameEngine::SetEmpireGameProperty (int iGameClass, int iGameNumber, int iEmpireKey, const char* pszColumn, const Variant& vProperty) {
    ...
    return t_pConn->WriteData (strGameEmpireData, pszColumn, vProperty);
}
