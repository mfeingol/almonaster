...
int GameEngine::BuildNewShips (int iGameClass, int iGameNumber, int iEmpireKey, int iTechKey, unsigned int iNumShips, 
    ...
    *pbBuildReduced = false;
    ...
    if (iTechKey == COLONY) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::PopLostToColonies, &vPopLostToColonies);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (iFleetKey != NO_KEY) {
        ...
        iErrCode = t_pConn->DoesRowExist (strEmpireFleets, iFleetKey, &bExists);
    ...
    if (vTemp.GetInteger() != INFINITE_SHIPS) {
        ...
        iErrCode = t_pConn->GetNumRows(strEmpireShips, &iNumAvailableShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    *piNumShipsBuilt = iNumShips;
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TechDevs, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TechLevel, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->InsertDuplicateRows (strEmpireShips, GameEmpireShips::Template, pvColVal, iNumShips);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::NumBuilds, iNumShips);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iTechKey == CLOAKER && (vShipBehavior.GetInteger() & CLOAKER_CLOAK_ON_BUILD)) {
        ...
        iErrCode = t_pConn->Increment (strEmpireMap, iProxyPlanetKey, GameEmpireMap::NumCloakedBuildShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->Increment (strGameMap, iPlanetKey, GameMap::NumCloakedBuildShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strEmpireMap, iProxyPlanetKey, GameEmpireMap::NumUncloakedBuildShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->Increment (strGameMap, iPlanetKey, GameMap::NumUncloakedBuildShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (iFleetKey != NO_KEY) {
        ...
        iErrCode = t_pConn->Increment (strEmpireFleets, iFleetKey, GameEmpireFleets::CurrentStrength, fMil);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->Increment (strEmpireFleets, iFleetKey, GameEmpireFleets::MaxStrength, fMil);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strEmpireFleets, iFleetKey, GameEmpireFleets::BuildShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strEmpireFleets, iFleetKey, GameEmpireFleets::NumShips, iNumShips);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetNumBuilds (int iGameClass, int iGameNumber, int iEmpireKey, int* piNumBuilds) {
    ...
    int iErrCode = t_pConn->GetCache()->ReadData(pszEmpireData, GameEmpireData::NumBuilds, &vTemp);
    if (iErrCode == OK) {
...
int GameEngine::GetBuildLocations (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piNumLocations = 0;
    ...
    iErrCode = t_pConn->GetNumRows(pszFleets, &iNumFleets);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
