...
int GameEngine::DeleteShip (int iGameClass, int iGameNumber, int iEmpireKey, int iShipKey) {
    ...
    if (iEmpireKey == INDEPENDENT) {
        ...
        // Get ship's locaton
        Variant vPlanetKey;
        iErrCode = t_pConn->GetCache()->ReadData(strIndependentShips, iShipKey, GameEmpireShips::CurrentPlanet, &vPlanetKey);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->Increment (strGameMap, vPlanetKey, GameMap::NumUncloakedShips, -1);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
#ifdef _DEBUG
        Variant vTemp;
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, vPlanetKey, GameMap::NumUncloakedShips, &vTemp);
        Assert (iErrCode == OK && vTemp >= 0);
#endif

        iErrCode = t_pConn->DeleteRow (strIndependentShips, iShipKey);

        if (iErrCode != OK) {
            Assert (false);
            
            // Compensate
            iErrCode2 = t_pConn->Increment (strGameMap, vPlanetKey, GameMap::NumUncloakedShips, 1);
            Assert (iErrCode2 == OK);
        }
    ...
    // Was ship just built?
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::BuiltThisUpdate, &vCancelBuild);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    // Was ship in a fleet?
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::FleetKey, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    // Was ship cloaked?
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::State, &vState);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    // Get ship's type
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::Type, &vTechKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    // Get ship's locaton
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::CurrentPlanet, &vPlanetKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }

    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::MaxBR, &vMaxBR);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iFleetKey != NO_KEY) {
        ...
        Variant vCurMil;
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::CurrentBR, &vCurMil);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (vCancelBuild.GetInteger() != 0) {
            iErrCode = t_pConn->Increment (strEmpireFleets, iFleetKey, GameEmpireFleets::BuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
    ...
    if (vTechKey.GetInteger() == COLONY && vCancelBuild.GetInteger() != 0) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::ColonyBuildCost, &vCost);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Minerals, &vMin);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Fuel, &vFuel);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Pop, &vPop);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::MaxPop, &vMaxPop);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TotalAg, &vTotalAg);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TotalPop, &vTotalPop);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        // Change next pop
        Variant vOldNextPop;
        iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::NextTotalPop, iNextPopDiff, &vOldNextPop);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (iDiff != 0) {
            iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::NextMin, iDiff);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
        ...
        if (iDiff != 0) {
            iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::NextFuel, iDiff);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
    ...
    iErrCode = t_pConn->GetFirstKey (strEmpireMap, GameEmpireMap::PlanetKey, vPlanetKey, &iKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (vCancelBuild.GetInteger() != 0) {
        ...
        iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::NumBuilds, -1);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::TotalBuild, - iBuildCost);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (vState.GetInteger() & CLOAKED) {
            ...
            iErrCode = t_pConn->Increment (strEmpireMap, iKey, GameEmpireMap::NumCloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            
            iErrCode = t_pConn->Increment (strGameMap, vPlanetKey, GameMap::NumCloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->Increment (strEmpireMap, iKey, GameEmpireMap::NumUncloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            
            iErrCode = t_pConn->Increment (strGameMap, vPlanetKey, GameMap::NumUncloakedBuildShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
        ...
        iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::TotalMaintenance, - iMaintCost);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pConn->Increment (strEmpireData, GameEmpireData::TotalFuelUse, - iFuelCost);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TotalFuelUse, &vTotalFuelUse);
        Assert (vTotalFuelUse.GetInteger() >= 0);
#endif
        if (vState.GetInteger() & CLOAKED) {
            
            iErrCode = t_pConn->Increment (strEmpireMap, iKey, GameEmpireMap::NumCloakedShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            
            iErrCode = t_pConn->Increment (strGameMap, vPlanetKey, GameMap::NumCloakedShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->Increment (strEmpireMap, iKey, GameEmpireMap::NumUncloakedShips, -1);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
    ...
    iErrCode = t_pConn->DeleteRow (strEmpireShips, iShipKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetRatioInformation (int iGameClass, int iGameNumber, int iEmpireKey, RatioInformation* pRatInfo) {
    ...
    iErrCode = t_pConn->GetTableForReading(strGameEmpireData, &pTable);
    if (iErrCode != OK) {
        return iErrCode;
    }
...
int GameEngine::GetShipOrders (unsigned int iGameClass, unsigned int iGameNumber, unsigned int iEmpireKey, 
    ...
    *piSelectedOrder = STAND_BY;
    ...
    if (pShipInfo != NULL) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::CurrentBR, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::MaxBR, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::Type, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::Action, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        // Get ship state
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::State, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::BuiltThisUpdate, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    if (pPlanetInfo != NULL) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::CurrentPlanet, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Name, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Coordinates, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    iErrCode = t_pConn->GetNumRows (strEmpireFleets, &iNumFleets);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->GetNumRows (strEmpireMap, &iMaxNumPlanets);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->GetFirstKey (strEmpireMap, GameEmpireMap::PlanetKey, iPlanetKey, &iProxyPlanetKey);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (bMobile) {
        ...
        // Read explored
        iErrCode = t_pConn->GetCache()->ReadData(strEmpireMap, iProxyPlanetKey, GameEmpireMap::Explored, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Link, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        ENUMERATE_CARDINAL_POINTS(i) {
            ...
            if (pbLink[i]) {
                ...
                if (!pbMustExplore[i]) {
                    ...
                    // Read planet key
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::NorthPlanetKey + i, &vTemp);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    ...
                    // Get name
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iNewPlanetKey, GameMap::Name, &vTemp);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
        ...
        if (iShipType != SCIENCE || !(iGameClassOptions & DISABLE_SCIENCE_SHIPS_NUKING)) {
            ...
            if (iPlanetOwner != iEmpireKey && iPlanetOwner != SYSTEM) {
                ...
                if (iPlanetOwner != INDEPENDENT) {
                    ...
                    unsigned int iKey;
                    iErrCode = t_pConn->GetFirstKey(strEmpireDip, GameEmpireDiplomacy::EmpireKey, iPlanetOwner, &iKey);
                    if (iErrCode == ERROR_DATA_NOT_FOUND) {
                        ...
                        iErrCode = t_pConn->GetCache()->ReadData(strEmpireDip, iKey, GameEmpireDiplomacy::CurrentStatus, &vTemp);
                        if (iErrCode != OK) {
                            Assert (false);
                            goto Cleanup;
                        }
    ...
    case STARGATE:
        ...
        if (fNextBR >= gcConfig.fStargateGateCost) {
            ...
            for (i = 0; i < iNumPlanets; i ++) {
                ...
                // Get coordinates
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iKey, GameMap::Coordinates, &vTemp);
                if (iErrCode != OK) {
                    Assert (false);
                ...
                if (
                    ...
                    // Get name
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iKey, GameMap::Name, &vTemp);
                    if (iErrCode != OK) {
                        Assert (false);
                        t_pConn->FreeKeys(piKey);
                        goto Cleanup;
            ...
            ENUMERATE_CARDINAL_POINTS(i) {
                ...
                if (pbLink[i]) {
                    ...
                    if (!(iExplored & EXPLORED_X[i])) {
                        ...
                        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, vTemp.GetInteger(), GameMap::Name, &vTemp);
                        if (iErrCode != OK) {
                            Assert (false);
                            goto Cleanup;
                        }
            ...
            for (i = 0; i < iNumPlanets; i ++) {
                ...
                if (iKey != iPlanetKey) {
                    ...
                    if (
                        ...
                        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iKey, GameMap::Annihilated, &vTemp);
                        if (iErrCode != OK) {
                            Assert (false);
                            t_pConn->FreeData(pvPlanetKey);
                            goto Cleanup;
    ...
    if ((iState & MORPH_ENABLED) && 
        ...
        // Get developed techs
        iErrCode = t_pConn->GetCache()->ReadData(strGameEmpireData, GameEmpireData::TechDevs, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::UpdateShipName (int iGameClass, int iGameNumber, int iEmpireKey, int iShipKey, 
    ...
    // Check ship key
    bool bShipExists;
    if (t_pConn->DoesRowExist (strEmpireShips, iShipKey, &bShipExists) != OK || !bShipExists) {
        return ERROR_SHIP_DOES_NOT_EXIST;
    }

    // Write new name
    return t_pConn->WriteData (strEmpireShips, iShipKey, GameEmpireShips::Name, pszNewName);
}
...
int GameEngine::UpdateShipOrders (unsigned int iGameClass, unsigned int iGameNumber, unsigned int iEmpireKey, 
    ...
    iErrCode = t_pConn->DoesRowExist (strEmpireShips, iShipKey, &bShipExists);
    ...
    if (vTemp.GetInteger() != 0) {
        ...
        if (iNewShipOrder == CANCEL_BUILD) {
            ...
            Variant vCurrentBR;
            iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::CurrentBR, &vCurrentBR);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            if (iNewShipOrder == BUILD_AT) {
                ...
                iErrCode = t_pConn->WriteData (strEmpireShips, iShipKey, GameEmpireShips::Action, BUILD_AT);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
                ...
                if (iOldFleetKey != NO_KEY) {
                    ...
                    iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::CurrentStrength, -fMil);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    
                    iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::MaxStrength, -fMil);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    ...
                    iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::BuildShips, -1);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    ...
                    iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::NumShips, -1);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    ...
                    // Set no fleet key
                    iErrCode = t_pConn->WriteData (strEmpireShips, iShipKey, GameEmpireShips::FleetKey, NO_KEY);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                ...
                iErrCode = t_pConn->DoesRowExist (strEmpireFleets, iNewShipOrder, &bShipExists);
                if (iErrCode != OK || !bShipExists) {
                ...
                iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::CurrentStrength, fMil);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
                
                iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::MaxStrength, fMil);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
                ...
                iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::BuildShips, 1);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
                ...
                iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::NumShips, 1);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
    ...
    if (IsMobileShip (iShipType) && iNewShipOrder >= 0) {
        ...
        iErrCode = t_pConn->WriteData (strEmpireShips, iShipKey, GameEmpireShips::FleetKey, iNewShipOrder);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::CurrentStrength, fCurrentMil);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        float fMaxMil = vMaxBR.GetFloat() * vMaxBR.GetFloat();
        iErrCode = t_pConn->Increment (strEmpireFleets, iNewShipOrder, GameEmpireFleets::MaxStrength, fMaxMil);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        if (iOldFleetKey != NO_KEY) {
            ...
            iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::CurrentStrength, -fCurrentMil);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            
            iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::MaxStrength, -fMaxMil);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        case LEAVE_FLEET:

            {
                ...
                iErrCode = t_pConn->Increment (strEmpireFleets, iOldFleetKey, GameEmpireFleets::MaxStrength, -fCurrentMil);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
        ...
        case MOVE_WEST:

            {
                ...
                Variant vLink;
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Link, &vLink);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
        ...
        case COLONIZE:
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Pop, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
            ...
            iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Annihilated, &vTemp);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
        ...
        case TERRAFORM:
            ...
            if ((iShipBehavior & TERRAFORMER_DISABLE_SURVIVAL) && !bDismantle) {
                ...
                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Ag, &vAg);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }

                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Fuel, &vFuel);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }

                iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::Minerals, &vMin);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
        ...
        case INVADE:
            ...
            if (iPlanetOwner != INDEPENDENT) {
                ...
                iErrCode = t_pConn->GetCache()->ReadData(strDiplomacy, iKey, GameEmpireDiplomacy::CurrentStatus, &vDipStatus);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
        ...
        case ANNIHILATE:
            {
                ...
                if (iPlanetOwner != SYSTEM && iPlanetOwner != INDEPENDENT) {
                    ...
                    if (iPlanetOwner == iEmpireKey) {
                        ...
                        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iShipPlanet, GameMap::HomeWorld, &vHW);
                        if (iErrCode != OK) {
                            Assert (false);
                            goto Cleanup;
                        }
        ...
        default:

            {
                ...
                // Make sure planet exists
                bool bPlanetExists;
                iErrCode = t_pConn->DoesRowExist (strGameMap, iNewShipOrder, &bPlanetExists);
                if (iErrCode != OK) {
                    Assert (false);
                    goto Cleanup;
                }
                ...
                if (iShipType == STARGATE) {
                    ...
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iNewShipOrder, GameMap::Owner, &vTemp);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
                    ...
                    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iNewShipOrder, GameMap::Annihilated, &vTemp);
                    if (iErrCode != OK) {
                        Assert (false);
                        goto Cleanup;
                    }
    ...
    iErrCode = t_pConn->WriteData (strEmpireShips, iShipKey, GameEmpireShips::Action, iNewShipOrder);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetNumShips (int iGameClass, int iGameNumber, int iEmpireKey, int* piNumShips) {
    ...
    return t_pConn->GetNumRows (pszShips, (unsigned int*) piNumShips);
}
...
int GameEngine::GetNumFleets (int iGameClass, int iGameNumber, int iEmpireKey, int* piNumFleets) {
    ...
    return t_pConn->GetNumRows (pszFleets, (unsigned int*) piNumFleets);
}
...
int GameEngine::ChangeShipCloakingState (int iShipKey, int iPlanetKey, bool bCloaked, 
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireShips, iShipKey, GameEmpireShips::State, &vOldState);
    Assert (iErrCode == OK);
    ...
    iErrCode = t_pConn->Increment (strEmpireMap, iProxyKey, GameEmpireMap::NumCloakedShips, iCloakedIncrement);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->Increment (strEmpireMap, iProxyKey, GameEmpireMap::NumUncloakedShips, iUnCloakedIncrement);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->Increment (strGameMap, iPlanetKey, GameMap::NumCloakedShips, iCloakedIncrement);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->Increment (strGameMap, iPlanetKey, GameMap::NumUncloakedShips, iUnCloakedIncrement);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireMap, iProxyKey, GameEmpireMap::NumUncloakedShips, &vNumShips);
    Assert (iErrCode == OK);
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireMap, iProxyKey, GameEmpireMap::NumCloakedShips, &vNumShips);
    Assert (iErrCode == OK);
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::NumUncloakedShips, &vNumShips);
    Assert (iErrCode == OK);
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::NumCloakedShips, &vNumShips);
    Assert (iErrCode == OK);
...
int GameEngine::GetColonyOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey,
    ...
    *pbColonize = *pbSettle = false;
    ...
    iErrCode = t_pConn->GetTableForReading(strGameMap, &pMap);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetTerraformerOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey,
    ...
    *pbTerraform = *pbTerraformAndDismantle = false;
    ...
    if (gcConfig.iShipBehavior & TERRAFORMER_DISABLE_FRIENDLY) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Owner, &vOwner);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Ag, &vAg);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    *pbInvade = *pbInvadeAndDismantle = false;
    ...
    if (vOwner.GetInteger() != (int) iEmpireKey && vOwner.GetInteger() != SYSTEM) {
        ...
        if (vOwner.GetInteger() == INDEPENDENT) {
            ...
            iErrCode = t_pConn->GetFirstKey (strEmpireDip, GameEmpireDiplomacy::EmpireKey, vOwner, &iKey);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }

            iErrCode = t_pConn->GetCache()->ReadData(strEmpireDip, iKey, GameEmpireDiplomacy::CurrentStatus, &vDipStatus);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
...
int GameEngine::GetDoomsdayOrders (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey,
    ...
    *pbAnnihilate = false;
    ...
    if (vOwner.GetInteger() == (int) iEmpireKey) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::HomeWorld, &vHW);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::MoveShip (unsigned int iGameClass, int iGameNumber, unsigned int iEmpireKey, 
    ...
    iErrCode = t_pConn->GetTableForReading(pszShips, &pShips);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetUnaffiliatedMobileShipsAtPlanet (unsigned int iGameClass, unsigned int iGameNumber,
    ...
    SearchColumn sc[2];
    sc[0].pszColumn = GameEmpireShips::CurrentPlanet;
    sc[0].iFlags = 0;
    sc[0].vData = iPlanetKey;
    sc[0].vData2 = iPlanetKey;

    sc[1].pszColumn = GameEmpireShips::FleetKey;
    sc[1].iFlags = 0;
    sc[1].vData = NO_KEY;
    sc[1].vData2 = NO_KEY;
    ...
    *piNumShips = 0;
    ...
    iErrCode = t_pConn->GetTableForReading(pszShips, &pIReadShips);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::HasUnaffiliatedMobileShipsAtPlanet (unsigned int iGameClass, unsigned int iGameNumber,
    ...
    sc[0].pszColumn = GameEmpireShips::FleetKey;
    sc[0].iFlags = 0;
    sc[0].vData = NO_KEY;
    sc[0].vData2 = NO_KEY;

    sc[1].pszColumn = GameEmpireShips::CurrentPlanet;
    sc[1].iFlags = 0;
