...
int GameEngine::GetSystemProperty(const char* pszColumn, Variant* pvProperty) {
    ...
    return t_pConn->GetViews()->ReadData(SYSTEM_DATA, pszColumn, pvProperty);
}
...
int GameEngine::SetScoreForPrivilege (Privilege privLevel, float fScore) {
    ...
    case APPRENTICE:
        ...
        iErrCode = t_pConn->GetViews()->ReadData(SYSTEM_DATA, SystemData::AdeptScore, &vHigherScore);
        break;
    ...
    case ADEPT:
        ...
        iErrCode = t_pConn->GetViews()->ReadData(SYSTEM_DATA, SystemData::AdeptScore, &vLowerScore);
        break;
...
int GameEngine::GetScoreForPrivilege (Privilege privLevel, float* pfScore) {
    ...
    iErrCode = t_pConn->GetViews()->ReadData(SYSTEM_DATA, pszReadColumn, &vTemp);
    if (iErrCode == OK) {
...
int GameEngine::GetSystemOptions (int* piOptions) {
    ...
    int iErrCode = t_pConn->GetViews()->ReadData(SYSTEM_DATA, SystemData::Options, &vTemp);
    if (iErrCode == OK) {
...
int GameEngine::GetDefaultShipName (int iTech, Variant* pvShipName) {
    ...
    return t_pConn->GetViews()->ReadData(SYSTEM_DATA, SYSTEM_DATA_SHIP_NAME_COLUMN [iTech], pvShipName);
}
