...
int GameEngine::DeliverSystemMessage (int iEmpireKey, const Variant* pvData) {
    ...
    // Lock
    if (!m_pConn->DoesTableExist (strMessages)) {

        iErrCode = m_pConn->CreateTable (strMessages, SystemEmpireMessages::Template);
    ...
    iErrCode = m_pConn->GetTableForWriting (strMessages, &pMessages);
    if (iErrCode != OK) {
        goto Cleanup;
    }
...
int GameEngine::GetNumSystemMessages (int iEmpireKey, unsigned int* piNumber) {
    ...
    if (!m_pConn->DoesTableExist (pszMessages)) {
        *piNumber = 0;
        return OK;
    }

    return m_pConn->GetNumRows (pszMessages, piNumber);
}
...
int GameEngine::GetNumUnreadSystemMessages (int iEmpireKey, unsigned int* piNumber) {
    ...
    iErrCode = m_pConn->GetTableForReading (pszMessages, &pMessages);
...
int GameEngine::SendMessageToAll (int iEmpireKey, const char* pszMessage) {
    ...
    while (true) {
        ...
        iErrCode = m_pConn->GetNextKey (SYSTEM_EMPIRE_DATA, iDestKey, &iDestKey);
        if (iErrCode != OK) {
            if (iErrCode == ERROR_DATA_NOT_FOUND) {
                iErrCode = OK;
            } else Assert (false);
...
int GameEngine::DeleteSystemMessage (int iEmpireKey, unsigned int iKey) {
    ...
    return m_pConn->DeleteRow (pszMessages, (unsigned int) iKey);
}
...
int GameEngine::SendGameMessage (int iGameClass, int iGameNumber, int iEmpireKey, const char* pszMessage, 
    ...
    iErrCode = m_pConn->ReadData (strEmpireData, GameEmpireData::MaxNumGameMessages, &vTemp);
    if (iErrCode != OK) {
        return iErrCode;
    }
    const unsigned int iMaxNumMessages = vTemp.GetInteger();
...
int GameEngine::GetUnreadGameMessages (int iGameClass, int iGameNumber, int iEmpireKey, Variant*** pppvMessage, 
    ...
    *piNumMessages = 0;
    ...
    iErrCode = m_pConn->ReadData (strGameEmpireData, GameEmpireData::MaxNumGameMessages, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::DeleteGameMessage (int iGameClass, int iGameNumber, int iEmpireKey, unsigned int iKey) {
    ...
    return m_pConn->DeleteRow (pszMessages, iKey);
}
...
int GameEngine::GetMessageProperty (const char* strMessages, unsigned int iMessageKey, const char* pszColumn, 
    ...
    iErrCode = m_pConn->GetTableForReading (strMessages, &pMessages);
    if (iErrCode != OK) {
        return iErrCode;
    }
