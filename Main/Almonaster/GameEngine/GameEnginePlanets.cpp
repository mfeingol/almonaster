...
int GameEngine::GetEmpirePlanetIcons (int iEmpireKey, unsigned int* piLivePlanetKey, 
    ...
    if (vTemp.GetInteger() == INDIVIDUAL_ELEMENTS)
    {
        ...
        iErrCode = t_pCache->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::UILivePlanet, &vTemp);
        if (iErrCode != OK) {
            return iErrCode;
        }
        *piLivePlanetKey = vTemp.GetInteger();
        
        iErrCode = t_pCache->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::UIDeadPlanet, &vTemp);
        if (iErrCode != OK) {
            return iErrCode;
        }
...
Cleanup:
    ...
    if (pvGameClassData != NULL) {
        t_pCache->FreeData(pvGameClassData);
    }

    if (pvGameData != NULL) {
        t_pCache->FreeData(pvGameData);
    }
    ...
    if (ppvPlanetData != NULL) {
        t_pCache->FreeData(ppvPlanetData);
    }
...
int GameEngine::VerifyMap (int iGameClass, int iGameNumber) {
    ...
    if (!(iGameClassOptions & DISCONNECTED_MAP)) {
        ...
        iErrCode = DfsTraversePlanets (pGameMap, 0, pvLink, pbVisited, iNumPlanets);
        if (iErrCode != OK) {
            goto Cleanup;
        }
    ...
    for (i = 0; i < iNumMatches; i ++) {
        ...
        iErrCode = t_pCache->ReadData(strGameMap, piKey[i], GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
Cleanup:
    ...
    if (pvLink != NULL) {
        t_pCache->FreeData(pvLink);
    }

    if (pvCoord != NULL) {
        t_pCache->FreeData(pvCoord);
    }
...
int GameEngine::DfsTraversePlanets (ICachedTable* pGameMap, unsigned int iPlanetKey, Variant* pvLink, bool* pbVisited, unsigned int iNumPlanets)
{
...
int GameEngine::GetPlanetName (int iGameClass, int iGameNumber, int iPlanetKey, Variant* pvPlanetName) {
    ...
    return t_pCache->ReadData(strGameMap, iPlanetKey, GameMap::Name, pvPlanetName);
}
...
int GameEngine::GetNeighbourPlanetKey (int iGameClass, int iGameNumber, int iPlanetKey, int iDirection, 
    ...
    Variant vKey;
    int iErrCode = t_pCache->ReadData(strGameMap, iPlanetKey, GameMap::NorthPlanetKey + iDirection, &vKey);
...
int GameEngine::RenamePlanet (int iGameClass, int iGameNumber, int iEmpireKey, int iPlanetKey, 
    ...
    if (iErrCode == OK && vEmpireKey.GetInteger() == iEmpireKey) {
        return t_pCache->WriteData (strMap, iPlanetKey, GameMap::Name, pszNewName);
    }
...
int GameEngine::GetSpectatorGameInfo (int iGameClass, int iGameNumber, 
    ...
    ICachedTable* pReadTable = NULL, * pEmpMap = NULL;
    ...
    *piMapMinX = *piMapMaxX = *piMapMinY = *piMapMaxY = 0;
    ...
    iErrCode = t_pCache->ReadData(pszGameData, GameData::State, &vGameState);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (vGameClassOptions.GetInteger() & EXPOSED_MAP) {
        ...
        // Easy way out
        iErrCode = t_pCache->GetAllKeys (pszGameMap, ppiPlanetKey, piNumPlanets);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }

        iErrCode = t_pCache->GetTable(pszGameData, &pReadTable);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        iErrCode = t_pCache->ReadColumn (pszEmpireMap, GameEmpireMap::PlanetKey, &pvPlanetKey, &iNumPlanets);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
        ...
        for (i = 0; i < iNumEmpires; i ++) {
            ...
            iErrCode = t_pCache->GetTable(pszEmpireMap, &pEmpMap);
            if (iErrCode != OK) {
                Assert (false);
                goto Cleanup;
            }
...
Cleanup:
    ...
    if ((iErrCode != OK || *piNumPlanets == 0) && (*ppiPlanetKey) != NULL) {
        t_pCache->FreeKeys(*ppiPlanetKey);
        *ppiPlanetKey = NULL;
    }
...
int GameEngine::IsPlanetSpectatorVisible (int iGameClass, int iGameNumber, int iPlanetKey, bool* pbVisible) {
    ...
    iErrCode = t_pCache->DoesRowExist(pszGameMap, iPlanetKey, pbVisible);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    while (true) {
        ...
        iErrCode = t_pCache->ReadData(pszEmpires, iKey, GameEmpires::EmpireKey, &vEmpireKey);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
