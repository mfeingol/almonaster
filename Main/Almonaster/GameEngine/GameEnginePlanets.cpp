...
int GameEngine::GetPlanetProperty (int iGameClass, int iGameNumber, unsigned int iPlanetKey, 
    ...
    GAME_MAP (pszMap, iGameClass, iGameNumber);
    return t_pConn->GetCache()->ReadData(pszMap, iPlanetKey, pszProperty, pvProperty);
}
...
int GameEngine::GetEmpirePlanetIcons (int iEmpireKey, unsigned int* piLivePlanetKey, 
    ...
    if (vTemp.GetInteger() == INDIVIDUAL_ELEMENTS)
    {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::UILivePlanet, &vTemp);
        if (iErrCode != OK) {
            return iErrCode;
        }
        *piLivePlanetKey = vTemp.GetInteger();
        
        iErrCode = t_pConn->GetCache()->ReadData(strSystemEmpireData, iEmpireKey, SystemEmpireData::UIDeadPlanet, &vTemp);
        if (iErrCode != OK) {
            return iErrCode;
        }
...
int GameEngine::AddEmpiresToMap (int iGameClass, int iGameNumber, int* piEmpireKey, int iNumEmpires, 
    ...
    *pbCommit = false;
    ...
    for (i = 0; i < iNumEmpires; i ++) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(pszEmpireData, GameEmpireData::TotalAg, &vTotalAg);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::VerifyMap (int iGameClass, int iGameNumber) {
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::NumPlanetsPerEmpire, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::Options, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    for (i = 0; i < iNumMatches; i ++) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strGameMap, piKey[i], GameMap::Owner, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetPlanetNameWithSecurity (int iGameClass, int iGameNumber, int iEmpireKey, int iPlanetKey, 
    ...
    if (iErrCode == OK) {
        ...
        return t_pConn->GetCache()->ReadData(
            strGameMap, 
            iPlanetKey, 
            GameMap::Name, 
            pvPlanetName
...
int GameEngine::GetPlanetName (int iGameClass, int iGameNumber, int iPlanetKey, Variant* pvPlanetName) {
    ...
    return t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::Name, pvPlanetName);
}
...
int GameEngine::GetNeighbourPlanetKey (int iGameClass, int iGameNumber, int iPlanetKey, int iDirection, 
    ...
    Variant vKey;
    int iErrCode = t_pConn->GetCache()->ReadData(strGameMap, iPlanetKey, GameMap::NorthPlanetKey + iDirection, &vKey);
...
int GameEngine::RenamePlanet (int iGameClass, int iGameNumber, int iEmpireKey, int iPlanetKey, 
    ...
    Variant vEmpireKey;
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::Owner, &vEmpireKey);
...
int GameEngine::SetPlanetMaxPop (int iGameClass, int iGameNumber, int iEmpireKey, int iPlanetKey, 
    ...
    // Get old maxpop
    int iOldMaxPop;
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::MaxPop, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::Pop, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpireData, GameEmpireData::TotalPop, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::PopLostToColonies, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::Minerals, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strMap, iPlanetKey, GameMap::Fuel, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
...
int GameEngine::GetGameAveragePlanetResources (int iGameClass, int iGameNumber, int* piAg, int* piMin, 
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::AvgAg, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    *piAg = vTemp.GetInteger();
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::AvgMin, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    *piMin = vTemp.GetInteger();
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strGameData, GameData::AvgFuel, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
...
int GameEngine::SetNewMinMaxIfNecessary (int iGameClass, int iGameNumber, int iEmpireKey, int iX, int iY) {
    ...
    iErrCode = t_pConn->GetCache()->ReadData(pszTable, iMinXCol, &vMinX);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->GetCache()->ReadData(pszTable, iMaxXCol, &vMaxX);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->GetCache()->ReadData(pszTable, iMinYCol, &vMinY);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    
    iErrCode = t_pConn->GetCache()->ReadData(pszTable, iMaxYCol, &vMaxY);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetSpectatorGameInfo (int iGameClass, int iGameNumber, 
    ...
    *piMapMinX = *piMapMaxX = *piMapMinY = *piMapMaxY = 0;
    ...
    iErrCode = t_pConn->GetCache()->ReadData(pszGameData, GameData::State, &vGameState);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
...
int GameEngine::IsPlanetSpectatorVisible (int iGameClass, int iGameNumber, int iPlanetKey, bool* pbVisible) {
    ...
    while (true) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(pszEmpires, iKey, GameEmpires::EmpireKey, &vEmpireKey);
        if (iErrCode != OK) {
            Assert (false);
            return iErrCode;
        }
