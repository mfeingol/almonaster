...
int GameEngine::AddEmpiresToMap (int iGameClass, int iGameNumber, int* piEmpireKey, int iNumEmpires, 
    ...
    *pbCommit = false;
    ...
    // Read game data
    iErrCode = t_pConn->ReadRow(strGameData, &pvGameData);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
...
int GameEngine::GetSpectatorGameInfo (int iGameClass, int iGameNumber, 
    ...
    *piMapMinX = *piMapMaxX = *piMapMinY = *piMapMaxY = 0;
    ...
    // Get gameclass options
    iErrCode = t_pConn->ReadData (SYSTEM_GAMECLASS_DATA, iGameClass, SystemGameClassData::iOptions, &vGameClassOptions);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
