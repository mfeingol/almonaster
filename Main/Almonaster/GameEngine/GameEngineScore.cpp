...
int GameEngine::UpdateScoresOnNuke (int iNukerKey, int iNukedKey, const char* pszNukerName, 
    ...
    iErrCode = t_pConn->Increment (SYSTEM_EMPIRE_DATA, iNukedKey, SystemEmpireData::Nuked, 1);
    if (iErrCode != OK) {
        Assert (false);
        return iErrCode;
    }
    ...
    if (t_pConn->GetCache()->ReadData(strNukedEmpire, iNukedKey, SystemEmpireData::AlienKey, &vValue) == OK)
    {
    ...
    if (t_pConn->GetCache()->ReadData(strNukerEmpire, iNukerKey, SystemEmpireData::AlienKey, &vValue) == OK)
    {
...
int GameEngine::UpdateScoresOn30StyleSurrenderColonization (int iWinnerKey, int iPlanetKey, 
    ...
    // Get nuker alien key
    iErrCode = t_pConn->GetCache()->ReadData(strWinnerEmpire, iWinnerKey, SystemEmpireData::AlienKey, &vTemp);
    if (iErrCode != OK) {
        Assert (false);
        goto Cleanup;
    }
    ...
    if (iLoserKey == NO_KEY) {
        ...
        iErrCode = t_pConn->GetCache()->ReadData(strLoserEmpire, iLoserKey, SystemEmpireData::AlienKey, &vTemp);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
...
int GameEngine::GetNumEmpiresInNukeHistory (int iEmpireKey, int* piNumNukes, int* piNumNuked) {
    ...
    if (!t_pConn->DoesTableExist (strTable)) {
        *piNumNukes = 0;
    } else {
        iErrCode = t_pConn->GetNumRows (strTable, (unsigned int*) piNumNukes);
        if (iErrCode != OK) {
            return iErrCode;
        }
    }
    ...
    if (!t_pConn->DoesTableExist (strTable)) {
        *piNumNuked = 0;
        ...
        iErrCode = t_pConn->GetNumRows (strTable, (unsigned int*) piNumNuked);
        if (iErrCode != OK) {
            return iErrCode;
        }
    }
    ...
    *pppvNukedData = *pppvNukerData = NULL;
    ...
    if (iErrCode != OK && iErrCode != ERROR_UNKNOWN_TABLE_NAME && iErrCode != ERROR_DATA_NOT_FOUND) {
        ...
        if (*pppvNukedData != NULL) {
            t_pConn->FreeData(*pppvNukedData);
            *pppvNukedData = NULL;
        }
        return iErrCode;
    }
...
int GameEngine::CalculatePrivilegeLevel(int iEmpireKey) {
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpire, iEmpireKey, SystemEmpireData::Privilege, &vPrivilege);
    if (iErrCode != OK) {
        return iErrCode;
    }
    ...
    iErrCode = t_pConn->GetCache()->ReadData(strEmpire, iEmpireKey, SystemEmpireData::AlmonasterScore, &vScore);
    if (iErrCode != OK) {
        return iErrCode;
    }
    ...
    if (vPrivilege.GetInteger() == NOVICE) {
        ...
        if (vScore < vAdeptLevel && vScore >= vApprenticeLevel) {
            ...
            iErrCode = t_pConn->WriteData (SYSTEM_EMPIRE_DATA, iEmpireKey, SystemEmpireData::Privilege, APPRENTICE);
            if (iErrCode != OK) {
                return iErrCode;
            }
            ...
            // Advance to adept
            iErrCode = t_pConn->WriteData (SYSTEM_EMPIRE_DATA, iEmpireKey, SystemEmpireData::Privilege, ADEPT);         
            if (iErrCode != OK) {
                return iErrCode;
            }
...
int GameEngine::AddNukeToHistory (NukeList nlNukeList, const char* pszGameClassName, int iGameNumber, 
    ...
    if (!t_pConn->DoesTableExist (pszTable)) {
        ...
        iErrCode = t_pConn->CreateTable (pszTable, *pttTemplate);
        if (iErrCode != OK) {
            Assert (false);
            goto Cleanup;
        }
    ...
    while (true) {
        ...
        if (sActivityDiff > 3 * 30 * DAY_LENGTH_IN_SECONDS) {
            ...
            if (iFinalIndex != vIndex.GetInteger()) {
                ...
                iErrCode = t_pConn->WriteData (SYSTEM_EMPIRE_DATA, iKey, SystemEmpireData::BridierIndex, iFinalIndex);
                if (iErrCode == OK) {
...
int GameEngine::SetBridierTimeBombScanFrequency (Seconds iFrequency) {
    ...
    return t_pConn->WriteData (SYSTEM_DATA, SystemData::BridierTimeBombScanFrequency, (int) iFrequency);
}