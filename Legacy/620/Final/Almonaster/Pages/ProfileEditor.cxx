...
case 0:
    ...
    if (bFlag) {
        %> selected<%
    } %> value="1">Send a message when my empire nukes or is nuked</option><option<%

    if (!bFlag) {
        %> selected<%
    } %> value="0">Don't send a message when my empire nukes or is nuked</option></select></td></tr><%


    %><tr><td>Send update messages on obliteration:</td><td><select name="DisplayFatalUpdates"><option<%

    bFlag = (pvEmpireData[SystemEmpireData::Options].GetInteger() & DISPLAY_FATAL_UPDATE_MESSAGES) != 0;

    if (bFlag) {
        %> selected<%
    } %> value="1">Send update messages when my empire is obliterated</option><option<%

    if (!bFlag) {
        %> selected<%
    } %> value="0">Don't send update messages when my empire is obliterated</option></select></td></tr><%
