...
int HtmlRenderer::Render_Planets() {
	...
	if (bMapGenerated) {
	    ...
	    if (iNumPlanets > 0) {
	        ...
	        for (i = 0; i < iNumPlanets; i ++) {
	            ...
	            iErrCode = pDatabase->ReadRow (strGameMap, pvPlanetKey[i].GetInteger(), &pvPlanetData);
	            if (iErrCode != OK) {
	                Assert (false);
	                goto Cleanup;
	            }
	...
	Write ("<input type=\"hidden\" name=\"NumOurPlanets\" value=\"", sizeof ("<input type=\"hidden\" name=\"NumOurPlanets\" value=\"") - 1);
	Write (iCounter); 
	Write ("\">", sizeof ("\">") - 1);
	}
