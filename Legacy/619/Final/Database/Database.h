...
typedef enum SearchType {
	SUBSTRING_SEARCH_CASE_SENSITIVE, 
	SUBSTRING_SEARCH_CASE_INSENSITIVE,
	EXACT_SEARCH_CASE_SENSITIVE,
	EXACT_SEARCH_CASE_INSENSITIVE,
	BEGINS_WITH_SEARCH_CASE_SENSITIVE,
	BEGINS_WITH_SEARCH_CASE_INSENSITIVE
};

#define INITIAL_INSERTION_LENGTH (0xffffffff)
...
#define ERROR_UNKNOWN_COLUMN_NAME (-1000006)
...
#define ERROR_COULD_NOT_EXECUTE_TRANSACTION (-1000028)
#define ERROR_TRANSACTION_CANCELLED (-1000029)

#define ERROR_DATABASE_HAS_NO_TABLES (-1000031)
#define ERROR_DATABASE_HAS_NO_TEMPLATES (-1000032)
#define ERROR_DATABASE_HAS_NO_BACKUPS (-1000033)
#define ERROR_COULD_NOT_CREATE_TEMPLATE_TABLE_DIRECTORY (-1000034)
#define ERROR_INITIAL_INSERTION_LENGTH_NOT_SPECIFIED (-1000035)
...
#define UNLOAD_EMPTY_TABLES	(0x00000002)
#define CHECK_ON_STARTUP	(0x00000004)
...
public:
	...
	virtual int GetSearchKeys (unsigned int iNumColumns, const unsigned int* piColumn, const Variant* pvData, 
		const Variant* pvData2, unsigned int iStartKey, unsigned int iSkipHits, unsigned int iMaxNumHits, 
		unsigned int** ppiKey, unsigned int* piNumHits, unsigned int* piStopKey) = 0;
...
class IWriteTable : virtual public IObject {
public:
	...
	virtual int InsertRow (Variant* pvColVal, unsigned int* piKey) = 0;
	virtual int InsertRow (Variant* pvColVal) = 0;

	virtual int InsertRows (Variant* pvColVal, unsigned int iNumRows) = 0;
	virtual int InsertDuplicateRows (Variant* pvColVal, unsigned int iNumRows) = 0;
...
public:
	...
	virtual int Backup (IDatabaseBackupNotificationSink* pSink) = 0;
	...
	virtual unsigned int GetNumLoadedRows() = 0;
	virtual size_t GetSizeOnDisk() = 0;
	...
	virtual int InsertRow (const char* pszTableName, Variant* pvColVal) = 0;
	virtual int InsertRow (const char* pszTableName, Variant* pvColVal, unsigned int* piKey) = 0;
	virtual int InsertRows (const char* pszTableName, Variant* pvColVal, unsigned int iNumRows) = 0;
	virtual int InsertDuplicateRows (const char* pszTableName, Variant* pvColVal, unsigned int iNumRows) = 0;
